...
RUN (mkdir -p /build/flux-file-storage-rest-api/libs/flux-autoload-api && cd /build/flux-file-storage-rest-api/libs/flux-autoload-api && wget -O - https://github.com/fluxfw/flux-autoload-api/releases/download/v2022-07-12-1/flux-autoload-api-v2022-07-12-1-build.tar.gz | tar -xz --strip-components=1 && /flux-namespace-changer/bin/change-namespace.php . FluxAutoloadApi FluxFileStorageRestApi\\Libs\\FluxAutoloadApi)

RUN (mkdir -p /build/flux-file-storage-rest-api/libs/flux-file-storage-api && cd /build/flux-file-storage-rest-api/libs/flux-file-storage-api && wget -O - https://github.com/fluxfw/flux-file-storage-api/releases/download/v2022-07-12-1/flux-file-storage-api-v2022-07-12-1-build.tar.gz | tar -xz --strip-components=1 && /flux-namespace-changer/bin/change-namespace.php . FluxFileStorageApi FluxFileStorageRestApi\\Libs\\FluxFileStorageApi)

RUN (mkdir -p /build/flux-file-storage-rest-api/libs/flux-rest-api && cd /build/flux-file-storage-rest-api/libs/flux-rest-api && wget -O - https://github.com/fluxfw/flux-rest-api/releases/download/v2022-07-12-1/flux-rest-api-v2022-07-12-1-build.tar.gz | tar -xz --strip-components=1 && /flux-namespace-changer/bin/change-namespace.php . FluxRestApi FluxFileStorageRestApi\\Libs\\FluxRestApi)

COPY . /build/flux-file-storage-rest-api

FROM php:8.1-cli-alpine

LABEL org.opencontainers.image.source="https://github.com/fluxfw/flux-file-storage-rest-api"
